...
function renderAssistantManager() {
  const assistantRows = Array.isArray(state.fullstackAgents) && state.fullstackAgents.length
    ? state.fullstackAgents
    : [
        { name: "canna测试", description: "Demo 智能体", model: "默认模型", status: "active" },
        { name: "演示AI助手", description: "Demo 智能体", model: "默认模型", status: "active" },
      ];
  return `<div class="module-content">
    ...
    <div class="assistant-card-row">
      ${assistantRows
        .map(
          (agent) => `
        <div class="card assistant-card" data-open-assistant="${escapeHtml(agent.name)}">
          <div style="display:flex; gap:12px; align-items:center">
            ${iconBox("AI")}
            <div>
              <div style="font-weight:700">${escapeHtml(agent.name)}</div>
              <div class="subtle">${escapeHtml(agent.description || agent.model || "")}</div>
            </div>
          </div>
          <span class="subtle">${agent.status === "active" ? "在线" : "暂停"}</span>
        </div>`
        )
        .join("")}
    </div>
